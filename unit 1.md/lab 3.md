...
# 🔐 Sudo and Admin Management Commands
...
👉 You’ll usually be asked to enter your password.
...
```bash
sudo usermod -aG sudo chaya     # Give 'chaya' sudo access  
```
...
```bash
sudo chown chaya:chaya myfile.txt
```
...
Shows mounted partitions in human-readable sizes.
...
👉 Alternative: `htop` (better UI, requires install).
